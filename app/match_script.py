...
def bot_forever():
    try: 
        result = client.cli_login(username=LOGON_DETAILS['username'], password=LOGON_DETAILS['password'])
...
def api_call(doc, tournament_id, match_id):
    ...
    try: 
        client.logout()
        requests.put(f'{url_prod}/{route}', json={'players': players, 'match_outcome': match_outcome, 
        'radiant_team_score': radiant_team_score, 'dire_team_score': dire_team_score, 
        'duration': duration, 'match_id': match_id, 'token': "CiNaegh7kush7shei4Ee"})
